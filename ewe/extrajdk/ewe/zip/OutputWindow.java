...
class OutputWindow
{
  ...
  private int window_filled = 0;
	...
	/**
	* Returns true on success or false if the window is full.
	**/
  public boolean write(int abyte)
  {
    if (window_filled++ == WINDOW_SIZE) return false;
  ...
  public boolean repeat(int len, int dist)
  {
    if ((window_filled += len) > WINDOW_SIZE) return false;
      //throw new IllegalStateException("Window full");
	...
	if (len <= dist)
	  {
	    ewe.sys.Vm.copyArray(window, rep_start, window, window_end, len);
	    window_end += len;
	  }
	else
	  {
  ...
  {
		int ws = WINDOW_SIZE - window_filled;
		int ab = input.getAvailableBytes();
		if (ws < len) len = ws;
		if (ab < len) len = ab;
  ...
  public boolean copyDict(byte[] dict, int offset, int len)
  {
    if (window_filled > 0) return false;
    ...
    ewe.sys.Vm.copyArray(dict, offset, window, 0, len);
  ...
  public int copyOutput(byte[] output, int offset, int len)
  {
    ...
    if (tailLen > 0)
      {
	ewe.sys.Vm.copyArray(window, WINDOW_SIZE - tailLen,
			 output, offset, tailLen);
	offset += tailLen;
	len = copy_end;
      }
    ewe.sys.Vm.copyArray(window, copy_end - len, output, offset, len);
    window_filled -= copied;
    if (window_filled < 0) return 0;
      //throw new IllegalStateException();
    return copied;
  }
