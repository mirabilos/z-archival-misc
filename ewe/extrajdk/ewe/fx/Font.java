...
public Font(String name, int style, int size)
	{
	if (name.toLowerCase().endsWith(" new")) name = name.substring(0,name.length()-4);
	this.name = name;
	this.style = style;
	this.size = size;
	}
...
protected java.awt.Font getAWTFont()
	...
	if ((this.style & BOLD) != 0) style |= java.awt.Font.BOLD;
	if ((this.style & ITALIC) != 0) style |= java.awt.Font.ITALIC;
	//if ((this.style & UNDERLINE) != 0) style |= java.awt.Font.UNDERLINE;
	return new java.awt.Font(name, style,(this.size*8)/10);//this.size - 3);
	}
...
public static String [] listFonts(ISurface s)
{
