...
__IDSTRING(rcsid_mpixbuf, "$MirOS: contrib/hosted/ewe/vm/mpixbuf.c,v 1.5 2008/05/03 23:22:32 tg Exp $");
...
#ifdef GTK_VERSION_1_2
#define isWindowPixmap(WHAT) ((WHAT) == GDK_WINDOW_PIXMAP)
#else
#define isWindowPixmap(WHAT) (0)
#endif
...
*/
	...
	alpha = dest->hasAlpha;
	bpp = alpha ? 4 : 3;
	dest->pixels = malloc(width*height*bpp);
	rowstride = dest->rowstride = width*bpp;
...
*/
	...
	rgbconvert (image, (guchar *)(dest->pixels +
	...
	gdk_image_destroy(image);
	dest = (struct mPixbuf *)malloc(sizeof(struct mPixbuf));
	*dest = ret;
	return dest;
}

void destroyPixbuf(struct mPixbuf *pb)
{
	if (pb){
		if (pb->pixels) free(pb->pixels);
		free(pb);
	}
}
