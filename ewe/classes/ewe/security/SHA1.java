...
public class SHA1
{
	...
	{
		int ret = ((nValue << nBits) | (nValue >>> (32 - nBits)));
		return ret;
	}
...
String stateToString()
{
	StringBuffer sb = new StringBuffer();
		for (int nI = 0; nI < m_block.length; nI++)
		{
			sb.append(ewe.sys.Convert.intToHexString(m_block[nI])+" ");
		}
		return sb.toString();
}
	/**
	...
	* @param data the data to add
	* @param nOfs the offset of the bytes in the data array.
	* @param nLen the number of bytes to add.
	*/
	public void update(
		byte[] data,
		int nOfs,
	...
	//===================================================================
	public byte[] digest(byte[] source,int offset,int length)
	//===================================================================
	{
		update(source,offset,length);
		digest();
		return getDigest();
	}
	//===================================================================
	...
	//===================================================================
	{
		return digest(source,0,source.length);
	}
	/**
	 ...
	 * @return the digset bytes as an array in DIGEST_SIZE bytes.
	 */
	public byte[] getDigest()
	{
		byte[] result = new byte[DIGEST_SIZE];
		ewe.sys.Vm.copyArray(m_digestBits, 0, result, 0, DIGEST_SIZE);
		return result;
	}
	...
	//===================================================================
	{
		if (otherDigest == null || length != DIGEST_SIZE) return false;
		for (int i = 0; i<length; i++)
			if (m_digestBits[i]  != otherDigest[offset+i]) return false;
		return true;
	}
	//===================================================================
	...
	//===================================================================
	{
		return equals(otherDigest,0,otherDigest.length);
	}
	...
	public int getDigest(
		...
		int nOfs)
	{
		ewe.sys.Vm.copyArray(m_digestBits, 0, buf, nOfs, DIGEST_SIZE);
		return DIGEST_SIZE;
	}
