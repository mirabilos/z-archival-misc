...
//===================================================================
{
	reset(savedKey,0,savedKey.length);
}
/**
* Reset the Encryptor/Decryptor using a specific key.
**/
//===================================================================
public void reset(byte[] key,int nOfs,int nlen)
//===================================================================
{
	if (backed == null){
		initialize(key,nOfs,nlen);
	}else {
		ewe.sys.Vm.copyArray(backed[0],0,m_pbox,0,m_pbox.length);
		ewe.sys.Vm.copyArray(backed[1],0,m_sbox1,0,m_sbox1.length);
		ewe.sys.Vm.copyArray(backed[2],0,m_sbox2,0,m_sbox2.length);
		ewe.sys.Vm.copyArray(backed[3],0,m_sbox3,0,m_sbox3.length);
		ewe.sys.Vm.copyArray(backed[4],0,m_sbox4,0,m_sbox4.length);
		ewe.sys.Vm.copyArray(backed[5],0,m_blockBuf,0,m_blockBuf.length);
		m_nWeakKey = -1;
	}
}
	...
	/**
	 * Initializes the instances, but does not really reset it since the PBOXes are not reset. Use
	 * the reset() methods to reset it.
		...
		int nLen)
	{
		...
		savedKey = new byte[nLen];
		ewe.sys.Vm.copyArray(key,nOfs,savedKey,0,nLen);
		...
		for (nI = 0; nI < PBOX_ENTRIES;)
		{
			...
			m_pbox[nI++] = byteArrayToInt(m_blockBuf, 0);
			m_pbox[nI++] = byteArrayToInt(m_blockBuf, 4);
		}

		for (nI = 0; nI < SBOX_ENTRIES;)
		{
			encryptPrv(m_blockBuf, 0, m_blockBuf, 0, BLOCKSIZE);
			m_sbox1[nI++] = byteArrayToInt(m_blockBuf, 0);
			m_sbox1[nI++] = byteArrayToInt(m_blockBuf, 4);
		}

		for (nI = 0; nI < SBOX_ENTRIES;)
		{
			encryptPrv(m_blockBuf, 0, m_blockBuf, 0, BLOCKSIZE);
			m_sbox2[nI++] = byteArrayToInt(m_blockBuf, 0);
			m_sbox2[nI++] = byteArrayToInt(m_blockBuf, 4);
		}

		for (nI = 0; nI < SBOX_ENTRIES;)
		{
			encryptPrv(m_blockBuf, 0, m_blockBuf, 0, BLOCKSIZE);
			m_sbox3[nI++] = byteArrayToInt(m_blockBuf, 0);
			m_sbox3[nI++] = byteArrayToInt(m_blockBuf, 4);
		}

		for (nI = 0; nI < SBOX_ENTRIES;)
		{
			encryptPrv(m_blockBuf, 0, m_blockBuf, 0, BLOCKSIZE);
			m_sbox4[nI++] = byteArrayToInt(m_blockBuf, 0);
			m_sbox4[nI++] = byteArrayToInt(m_blockBuf, 4);
		}
		...
		if (backed == null){
			backed = new Object[6];
			backed[0] = m_pbox.clone();
			backed[1] = m_sbox1.clone();
			backed[2] = m_sbox2.clone();
			backed[3] = m_sbox3.clone();
			backed[4] = m_sbox4.clone();
			backed[5] = m_blockBuf.clone();
		}
	}
...
*/
	...
	//-------------------------------------------------------------------
	private Object zero(Object array)
	//-------------------------------------------------------------------
	{
		Utils.zeroArrayRegion(array,0,Array.getLength(array));
		return null;
	}
	/**
