...
//===================================================================
{
	add(ewe.sys.Vm.getStringChars(toAdd),0,toAdd.length());
}
//===================================================================
...
//===================================================================
{
	...
	if (replacers[0] instanceof String)
		for (int i = 0; i<replacers.length; i++)
			replacers[i] = ewe.sys.Vm.getStringChars((String)replacers[i]);
...
//===================================================================
{
	...
	for (int i = start; i<end; i++){
		if (chars[i] == '&'){
			...
			char toSub = '?';
			if (chars[i+1] == '#')
				toSub = (char)ewe.sys.Convert.toInt(new String(chars,i+2,e-i-2));
			else{
			...
			if (e != end-1) ewe.sys.Vm.copyArray(chars,e+1,chars,i+1,end-(e+1));
			chars[i] = toSub;
			end -= (e+1)-(i+1);
		}
	}
		...
		for (int i = start; i<end; i++){
			if (chars[i] == ' '){
				...
				if (first != -1 && first != i-1){
					ewe.sys.Vm.copyArray(chars,i,chars,first+1,end-i);
					end -= i-first-1;
				}
				first = -1;
			}
...
//===================================================================
{
	...
	char [] look = ewe.sys.Vm.getStringChars(styleName);

	for (int i = 0;;i++){
		i = SubString.indexOf(':',source,0,source.length,i,0);
		//ewe.sys.Vm.debug("Look: "+styleName+" at "+i);
		if (i == -1) return null;
		if (i == 0) continue;
		int e, s;
		...
		if (!SubString.equals(look,0,look.length,source,s,len,SubString.IGNORE_CASE))
			continue;
		//
		//ewe.sys.Vm.debug("Found: "+styleName);
		//
...
//===================================================================
{
	...
	char [] source = ewe.sys.Vm.getStringChars(attributes);
...
//-------------------------------------------------------------------
{
	...
	if (nowEntries > maxEntries) {
		//ewe.sys.Vm.debug("Max: "+nowEntries);
		...
		if (e.type == type) {
			...
			if (i != cs){
				while(true){
					...
					ewe.sys.Vm.debug("Force ending: "+valueToName(ty));
					end(ty);
				}
			}
			*/
...
//===================================================================
{
	...
	char [] st = style == null ? null : ewe.sys.Vm.getStringChars(style);
	...
	if (st != null){
		...
		switch(e.type){
			...
			case TELETYPE:
				...
				tf.fontName = mApp.findFont("fixed",true).getName();
				break;
			case BIG:
				tf.fontFlags |= tf.FONT_CHANGE;
				tf.fontSizeChange += 2;
			...
			case FONT:
				...
				if (size instanceof String){
					...
					if (siz.charAt(0) == '+' || siz.charAt(0) == '-'){
						int sz = ewe.sys.Convert.toInt(siz.substring(1));
						if (siz.charAt(0) == '-') sz *= -1;
						ea.set("sizeChange",new ewe.sys.Long().set(sz*2));
						size = null;
					}else{
						int sz = ewe.sys.Convert.toInt(siz);
						ea.set("size",size = new ewe.sys.Long().set(sz*2+12));
					}
				}
				if (size instanceof ewe.sys.Long){
					tf.fontSize = (int)((ewe.sys.Long)size).value;
				}
...
ewe.sys.Lock splitLock = new ewe.sys.Lock();
...
//===================================================================
{
	...
	try{
		...
		for (int i = 0; i<blanks.length; i+=2){
			ewe.sys.Vm.debug("Line: "+blanks.data[i]+", add: "+blanks.data[i+1]);
		}
		*/
		int max = v.size();
		for (int i = 0; i<max; i++){
		...
		new ReportException(e,null,null,false).show();
		//ewe.sys.Vm.debug(ewe.sys.Vm.getStackTrace(e,10));
	}
...
static int EndFormatterFlags = TextFormatter.LINE_CENTERED|TextFormatter.LINE_RIGHT_ALIGNED;
	...
	//##################################################################
	public class Entry extends ewe.data.LiveTreeNode{
	//##################################################################
	int startIndex;
	int length;
	...
	*/
	...
	/*
	//===================================================================
	public void getChildrenFormatters(mTextPad pad,TextPosition t,Vector dest)
	//===================================================================
	{
	...
	*/
	/*
	//===================================================================
	public void addBlankLines(mTextPad pad,IntArray blanks)
	//===================================================================
	{
		if (!hasImage) return;
		if (putLinesAbove != 0){
	...
	*/
	...
	//-------------------------------------------------------------------
	{
		//ewe.sys.Vm.debug(numInserted+" inserted above: "+startLine);
		if (start == null){
			...
			//ewe.sys.Vm.debug("Type: "+type+", I  was: "+sl+"->"+nl);
			//ewe.sys.Vm.debug("Type: "+type+", am now: "+formatter.startLine+"->"+formatter.numLines);
		}else{
			...
			//ewe.sys.Vm.debug(hashCode()+" Type: "+type+", I  was: "+sl+" @ "+nl);
			//ewe.sys.Vm.debug(hashCode()+" Type: "+type+", am now: "+start.lineIndex+" @ "+start.characterIndex);
		}
	}
	...
	//-------------------------------------------------------------------
	void addLineSpacers(mTextPad pad,int lineIndex,DisplayLine line,int above,int below,int [] indexes)
	//-------------------------------------------------------------------
	{
	...
	//-------------------------------------------------------------------
	void addLineSpacers(mTextPad pad,int [] indexes)
	//-------------------------------------------------------------------
	{
	...
	int myLineIndex;
	//-------------------------------------------------------------------
	void getLineSpacers(mTextPad pad,int lineIndex,DisplayLine line,Vector dest)
	//-------------------------------------------------------------------
	{
		if (!needPass2) return;
	...
	**/
	//===================================================================
	public boolean reformat(mTextPad pad,TextPosition t,Vector addTo)
	//===================================================================
	{
		try{
			start = t.getCopy();
			//ewe.sys.Vm.debug(type+" for: "+start.characterIndex+" on "+start.lineIndex);
			if (!start.findCharacter(startIndex)){
				//ewe.sys.Vm.debug("Not found: "+type+", "+startIndex);
				return false;
			}
			/*
			int depth = 0;
			...
			*/
			...
			if (type == IMAGE){
				...
				if (align.equals("bottom")){
					...
					//ewe.sys.Vm.debug(tf.image.getHeight()+", "+pad.getLineHeight()+", "+pad.getBaselineHeight());
				}
				//
				...
				if (align.equals("left") || align.equals("right")){
					...
					while(didLines < numLines){
						...
						pad.resplit(first,num,0,new DisplayLine.WidthProvider(){
							...
							public int getWidthFor(DisplayLineSpecs specs, int lineIndex, int startingCharacterIndex, DisplayLine splitSoFar)
							{
								...
								total++;
								//ewe.sys.Vm.debug(lineIndex+", "+ret+", "+fullWidth+", "+lineWidth);
								return ret;
							}
						}
							,null,null);
					...
					if (numLines != 1 || true){
						if (align.equals("top")) {
							...
							//ewe.sys.Vm.debug("Bottom: "+tf.image.getHeight()+", "+ih+", "+numLines+", "+tf.imageY);
						}
	...
	/*
	//===================================================================
	public boolean getTextFormatter(mTextPad pad,TextPosition t,Vector addTo)
	//===================================================================
	{
		start = t.getCopy();
...
/*
	//===================================================================
	public boolean getFormatter(mTextPad pad,TextPosition t,Vector dest)
	//===================================================================
	{
		...
		if (type == IMAGE){ //length will be zero!
			...
			int width = before == 0 ? 0 : fts.calculatedPositions[before-1];
			width += fts.leftMargin+pad.spacing;
			//ewe.sys.Vm.debug(start.line.displayWidth+", "+width+", "+extraWidth);
			int pw = pad.getSize(null).width;
			final int normalWidth = pw-(pad.spacing*2)-pad.rightMargin-fts.leftMargin;
			//ewe.sys.Vm.debug((pw-pad.spacing-pad.rightMargin)+", "+width+", "+extraWidth+", "+(displayWidth-extraWidth));
			if (width+extraWidth >= normalWidth){
				//ewe.sys.Vm.debug("Splitting!");
				DisplayLine dl = pad.breakLineBefore(start.line,startIndex-start.characterIndex);
				if (dl != null){
					start.lineIndex++;
					start.line = dl;
			...
			tf.imageLine = start.lineIndex;
			tf.imageChar = startIndex-start.characterIndex;
			//ewe.sys.Vm.debug(tf.imageLine+", "+tf.imageChar);
			tf.startCharacter = 0;
			tf.endCharacter = -1;
			...
			if (added > 0) {
				//ewe.sys.Vm.debug(hashCode()+" - adding: "+added);
			...
			//
			setupFormatter(start,t);
			if (type == ITALIC) ewe.sys.Vm.debug(formatter.toString());
		}
		start = null;
		return true;
	}
...
//===================================================================
{
	...
	if (data != null && e.type == OLI){
		...
		int value = ewe.sys.Convert.toInt(dt);
		if (value == 0 && !dt.equals("0")){
			...
			dt += ".";
			e.formatter.data = dt;
			dt = ewe.sys.Convert.toString(value+1);
		}
		s.data = dt;
	}
}
...
//===================================================================
{
	...
	splitLock.synchronize(); try{
	char [] source = ewe.sys.Vm.getStringChars(text);
	int len = source.length;
	String headerTag = null;
	boolean pastHeader = false;
	boolean inHeader = false;
	boolean inForm = false;
	if (handle == null) handle = new ewe.sys.Handle();
	...
	for(int reached = 0; reached < len && !handle.shouldStop;){
		...
		if (tag.equals("body")){
			if (!ending){
				...
				bodyData.set("background",toColor(a.getValue("bgcolor",Color.White)));
				bodyData.set("foreground",toColor(a.getValue("text",Color.Black)));
				bodyData.set("link",toColor(a.getValue("link",HotSpot.hotColor)));
			}
			continue;
		...
		ewe.sys.Vm.debug(ewe.sys.Vm.getStackTrace(e,10));
	}finally{splitLock.release(); if (handle != null) handle.setProgress(1.0f);}
}
//===================================================================
public void endHtml()
//===================================================================
...
//===================================================================
{
	...
	String path = ewe.io.File.removeTrailingSlash(properties.getString("documentRoot",""));
	...
	*/
		...
		//ewe.sys.Vm.debug("Bad image: "+path);
		//ewe.sys.Vm.debug(ewe.sys.Vm.getStackTrace(e,10));
	}
	return null;
}
