...
public class BufferedReader extends Reader
{
  ...
  public BufferedReader(Reader in, int size)
  {
	...
	public BufferedReader(BasicStream in)
	{
	...
	if (pos + readLimit > limit)
	  {
	    ...
	    if (limit >= 0)
	      {
	        ewe.sys.Vm.copyArray(old_buffer, pos, buffer, 0, limit);
	        pos = 0;
	      }
	  }
  ...
  public int read(char[] buf, int offset, int count) throws IOException
  {
	...
	ewe.sys.Vm.copyArray(buffer, pos, buf, offset, count);
	pos += count;
	return count;
     }finally{lock.release();}
  }
  ...
  public int read() throws IOException
  {
	...
	checkStatus();
	//
	//- Bug, if the read() in fill() returns 1 byte but there was a '\r' before,
	// then fill() returns 0.
	//if (pos >= limit && fill () <= 0)
  //		return -1;
	//
