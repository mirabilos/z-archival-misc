...
public String lineSeparator;
 /**
  ...
  {
		if (lineSeparator != null) write(lineSeparator);
    else write (ewe.sys.Vm.getProperty("line.separator","\n"));
  }
  ...
  public void write (char[] buf, int offset, int len) throws IOException
  {
	...
	if (count + len > buffer.length)
	  ...
	  {
	    ewe.sys.Vm.copyArray(buf, offset, buffer, count, len);
	    count += len;
	    if (count == buffer.length)
	      localFlush ();
	  }
