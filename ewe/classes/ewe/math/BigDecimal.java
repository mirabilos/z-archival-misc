...
public class BigDecimal extends Number implements ewe.util.Comparable, ByteEncodable
{
  ...
  {
    if (Number.isInfinite (num) || Number.isNaN (num))
    ...
    long bits = ewe.sys.Convert.toLongBitwise(num);
    ...
    if (point < len)
      {
        ...
        try
	  {
	    int exp = ewe.sys.Convert.toInt(num.substring (point));//Integer.parseInt (num.substring (point));
	    exp -= scale;
	    if (signum () == 0)
	      scale = 0;
	    else if (exp > 0)
  ...
  public BigDecimal add (BigDecimal val)
  {
    ...
    return new BigDecimal (op1.add (op2), java.lang.Math.max (scale, val.scale));
  }
...
//      System.out.println("rem: " + parts[1]);
  ...
  public String toString ()
  {
	...
	char [] all = ewe.sys.Vm.getStringChars(str);
	sb.setLength(0);
	int p = point+(negative?1:0);
	if (p != 0) sb.append(new String(all,0,p));
	sb.append('.');
	sb.append(new String(all,p,all.length-p));
  ...
  public float floatValue()
  {
   // return Float.valueOf(toString()).floatValue();
		return ewe.sys.Convert.toFloat(toString());
  }

  public double doubleValue()
  {
    return ewe.sys.Convert.toDouble(toString());//Double.valueOf(toString()).doubleValue();
  }
    ...
    throws ArithmeticException, IllegalArgumentException
  {
	...
	//===================================================================
	{
		int count = 4;
		if (data == null)
			return count+(intVal == null ? 0 : intVal.write(null,0));
		Utils.writeInt(scale,data,offset,4); offset += 4;
		if (intVal != null) count += intVal.write(data,offset);
		return count;
	}
	//===================================================================
	...
	/**
	 * Find out how many bytes constitute an encoded BigDecimal.
	 * @param data the data to read.
	 * @param offset the start of the data.
	 * @return the number of bytes representing the encoded BigDecimal.
	 */
		/*
	//===================================================================
	...
	//===================================================================
	{
		return 5 + (data[offset+4] != 0 ? BigInteger.sizeOfEncoded(data,offset+5) : 0);
	}
	*/
	//-------------------------------------------------------------------
	private void readFrom(byte[] data,int offset,int length)
	//-------------------------------------------------------------------
	{
		scale = Utils.readInt(data,offset,4); offset += 4;
		intVal = length > 4 ? new BigInteger(data,offset,length-4) : null;
	}
	/**
	...
	//-------------------------------------------------------------------
	static BigDecimal c1 = new BigDecimal(), c2 = new BigDecimal();
	//-------------------------------------------------------------------
	//===================================================================
	public static int compareEncoded(byte[] one, int oneOffset, int oneLen, byte[] two, int twoOffset,int twoLen)
	//===================================================================
	{
		BigDecimal o = c1, t = c2;
		if (oneLen <= 0) o = null;
		else o.readFrom(one,oneOffset,oneLen);
		if (twoLen <= 0) t = null;
		else t.readFrom(two,twoOffset,twoLen);
		if (o == t) return 0;
		if (o == null) return -1;
		if (t == null) return 1;
