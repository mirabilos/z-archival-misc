/* $MirOS: contrib/hosted/ewe/classes/ewe/util/Properties.java,v 1.2 2007/08/30 23:18:17 tg Exp $ */
...
public class Properties extends Hashtable
{
  ...
  // comments in vm/reference/java/lang/Runtime for implications of this fact.
	...
	/**
	* This is the encoding used. By default it is IO.JAVA_UTF8_CODEC
	**/
	public static String ENCODING = IO.JAVA_UTF8_CODEC; // "ISO-8859-1" <- The standard encoding.
  /**
  ...
  public Properties(Properties defaults)
  {
	 ...
	 * This method will also set the default values in the destination PropertyList if they
	 * are not explicitly set in this Properties object.
	 * @param destination A possibly null destination PropertyList.
	 * @return The destination or new PropertyList.
	 */
	//===================================================================
	...
	//===================================================================
	{
		...
		for (Iterator it = entries(); it.hasNext();){
			...
			destination.set((String)me.getKey(),me.getValue());
		}
		if (defaults != null)
			for (Iterator it = defaults.entries(); it.hasNext();){
				Map.MapEntry me = (Map.MapEntry)it.next();
				destination.defaultTo((String)me.getKey(),me.getValue());
			}
		return destination;
	}
	/**
	...
	**/
	...
	{
		if (other != null)
			for (Iterator it = other.entries(); it.hasNext();){
				Map.MapEntry me = (Map.MapEntry)it.next();
				setProperty((String)me.getKey(),(String)me.getValue());
			}
	}
  /**
                    ...
                    switch (c)
                      ...
                      case 'u':
                        if (pos + 4 <= line.length())
                          {
                            char uni = (char) Convert.parseInt
...
*/
  ...
  public void store(OutputStream out, String header) throws IOException
  {
    ...
    writer.println ("#" + new Time().toString());//Calendar.getInstance ().getTime ());
    ...
    while (--i >= 0)
      {
        Map.MapEntry entry = (Map.MapEntry) iter.next ();
  ...
  public String getProperty(String key, String defaultValue)
  {
	...
	//===================================================================
	{
		...
		for (Iterator it = entries(); it.hasNext();){
			Map.MapEntry me = (Map.MapEntry)it.next();
			p.setProperty((String)me.getKey(),(String)me.getValue());
		}
		return p;
	}
  /**
  ...
  public Enumeration propertyNames()
  {
    ...
    Properties prop = this;
		Vector v = new Vector();
		for (Iterator it = entries(); it.hasNext();){
			Map.MapEntry m = (Map.MapEntry)it.next();
			v.add(m.getKey());
		}
		return v.elements();
  }
    ...
    for (int i = 0; i < size; i++)
      {
        ...
        switch (c)
          ...
          default:
            if (c < ' ' || c > '~')
              {
                String hex = Convert.intToHexString(c);
